...
def test_weighing_feedback_loads_directly_after_weighing_card() -> None:
    """Weight feedback belongs to the weighing card, not a prototype patch."""
    assert WEIGHING_FEEDBACK not in CARD_FILES
...
def test_weighing_feedback_uses_previous_effective_weight() -> None:
    ...
    assert "const gainGrams = value - previousWeight;" in source
