// Puppy Weight Tracker Card v1.1.0
class PuppyWeightTrackerCard extends HTMLElement {
  ...
  set hass(hass) {
    ...
    // Home Assistant supplies a new hass object very frequently. Replacing the
    // complete shadow DOM on every update makes mobile browsers drop focus from
    // the numeric weight field. While the user is typing, keep the DOM stable
    // and render the newest HA state after the field loses focus.
    if (this._editingWeight || this.shadowRoot?.activeElement?.id === "weight-input") {
  ...
  _bindEvents(station) {
    ...
    litterSelect?.addEventListener("change", (event) => {
      this._draftWeight = null;
      this._select(station.ids.litter, event.target.value);
    });

    puppySelect?.addEventListener("change", (event) => {
      this._draftWeight = null;
      this._select(station.ids.puppy, event.target.value);
    });
    ...
    weightInput?.addEventListener("blur", () => {
      ...
      // Let a click/tap on the Save button finish before an outstanding render
      // is allowed to replace the DOM. This is particularly important on iOS.
      if (this._renderPending) {
        window.setTimeout(() => {
          if (!this._editingWeight) {
            this._renderPending = false;
            this._render();
          }
        }, 120);
    ...
    start?.addEventListener("click", () =>
      this._press(station.ids.start, "Weegsessie starten")
    );
    reset?.addEventListener("click", () =>
      this._press(station.ids.reset, "Weegsessie resetten")
    );

    this.shadowRoot?.querySelectorAll("[data-puppy-option]").forEach((row) => {
      row.addEventListener("click", () => {
        this._draftWeight = null;
        this._select(station.ids.puppy, row.dataset.puppyOption);
      });
    });
  }
