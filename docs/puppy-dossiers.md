...
The owner store is included in the integration runtime and has websocket commands for list, save, delete, and link operations. The PDF report card has a `Baasjes` section toggle; when enabled, linked contact names and available e-mail/telephone details are included in the report.
